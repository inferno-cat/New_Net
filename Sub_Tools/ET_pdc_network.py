...
#     def __init__(
#             self, in_channels, out_channels, kernel_size, stride=1, dilation=1, activation=None, use_bn=False
#     ):
...
#             padding=kernel_size // 2 * dilation, dilation=dilation, groups=in_channels
#         )
...
class MultiScaleContextModule(nn.Module):
    def __init__(self, dim):
        ...
        # self.attn = AT_CBAM(dim)
        # self.attn = AT_SESA(dim)
        # self.attn = EAT_CBAM(dim)
        # self.attn = AT_LWGA(
        #     dim=dim, mlp_ratio=4.0, drop_path=0.1
        # )
        # self.attn2 = AT_LWGA(
        #     dim=dim, mlp_ratio=4.0, drop_path=0.1
        # )
...
class Decoder(nn.Module):
    def __init__(self, in_channels):
        ...
        # self.conv2 = AT_SMFA(in_channels // 2)
        #
        # self.conv2_2 = AT_SMFA(in_channels // 2)
...
class PDCNet(nn.Module):
    def __init__(self, base_dim=16):
        ...
        # self.stem_conv = nn.Sequential(
        #     nn.Conv2d(3, self.in_channels[0], kernel_size=3, stride=1, padding=1, bias=False),
        #     AT_WTConv(self.in_channels[0], self.in_channels[0], kernel_size=3, stride=1, bias=False, wt_levels=1,
        #               wt_type='db1'),
        #     # nn.BatchNorm2d(self.in_channels[0]),
        #     # nn.ReLU(inplace=True)
        # )
        # self.stem_conv1 = nn.Conv2d(3, self.in_channels[0], kernel_size=3, stride=1, padding=1, bias=False)
        # self.stem_WTConv = AT_WTConv(self.in_channels[0], self.in_channels[0], kernel_size=3, stride=1, bias=False, wt_levels=1,
        #                              wt_type='db1')
        ...
        # self.down2 = AT_DownSample(self.in_channels[0], self.in_channels[1])
        # self.down3 = AT_DownSample(self.in_channels[1], self.in_channels[2])
        # self.down4 = AT_DownSample(self.in_channels[2], self.in_channels[3])

        # self.catfuse4 = AT_CatFuse(dim=self.in_channels[3], num_heads=4, group_split=[2, 2], kernel_sizes=[3], window_size=7)
        # self.catfuse3 = AT_CatFuse(dim=self.in_channels[2], num_heads=8, group_split=[4, 4], kernel_sizes=[3], window_size=7)
        # self.catfuse2 = AT_CatFuse(dim=self.in_channels[1], num_heads=8, group_split=[4, 4], kernel_sizes=[3], window_size=7)
        # self.catfuse1 = AT_CatFuse(dim=self.in_channels[0], num_heads=8, group_split=[4, 4], kernel_sizes=[3], window_size=7)

        # self.ssa4 = AT_SSA(size = 4, dim=self.in_channels[3],)
        # self.ssa3 = AT_SSA(size = 4, dim=self.in_channels[2],)
        # self.ssa2 = AT_SSA(size = 4, dim=self.in_channels[1],)
        # self.ssa1 = AT_SSA(size = 4, dim=self.in_channels[0],)
        #
        # self.ssa_down4_1 = AT_DownSample(self.in_channels[3], self.in_channels[3])
        # self.ssa_down4_2 = AT_DownSample(self.in_channels[3], self.in_channels[3])
        # self.ssa_down3_1 = AT_DownSample(self.in_channels[2], self.in_channels[3])
        # self.ssa_down3_2 = AT_DownSample(self.in_channels[3], self.in_channels[3])
        # self.ssa_down2_1 = AT_DownSample(self.in_channels[1], self.in_channels[1])
        # self.ssa_down2_2 = AT_DownSample(self.in_channels[1], self.in_channels[1])
        # self.ssa_down1_1 = AT_DownSample(self.in_channels[0], self.in_channels[0])
        # self.ssa_down1_2 = AT_DownSample(self.in_channels[0], self.in_channels[0])
        #
        # self.ssa_up4 = AT_UpSample(self.in_channels[3], self.in_channels[2])
        # self.ssa_up3 = AT_UpSample(self.in_channels[2], self.in_channels[1])
        # self.ssa_up2 = AT_UpSample(self.in_channels[1], self.in_channels[0])
        #
        # self.ssa_final_up_1 = AT_UpSample(self.in_channels[0], self.in_channels[0])
        # self.ssa_final_up_2 = AT_UpSample(self.in_channels[0], self.in_channels[0])

        # self.ssa_down1 = AT_DownSample(3, self.in_channels[0])
        # self.ssa_down2 = AT_DownSample(self.in_channels[0], self.in_channels[0])
        # self.ssa_down3 = AT_DownSample(self.in_channels[0], self.in_channels[0])
        # self.ssa = AT_SSA(size=4, dim=self.in_channels[0],)
        # self.ssa_up1 = AT_UpSample(self.in_channels[0], self.in_channels[0])
        # self.ssa_up2 = AT_UpSample(self.in_channels[0], self.in_channels[0])
        # self.ssa_up3 = AT_UpSample(self.in_channels[0], self.in_channels[0])
        ...
        # self.mscm4 = AT_MSB(self.in_channels[3])
        # self.mscm3 = AT_MSB(self.in_channels[2])
        # self.mscm2 = AT_MSB(self.in_channels[1])
        # self.mscm1 = AT_MSB(self.in_channels[0])
        ...
        # self.fuse_up4 = AT_UpSample(self.in_channels[3], self.in_channels[2])
        # self.fuse_up3 = AT_UpSample(self.in_channels[2], self.in_channels[1])
        # self.fuse_up2 = AT_UpSample(self.in_channels[1], self.in_channels[0])
        #
        # self.lca3 = AT_LCA(dim=self.in_channels[2], num_heads=4)
        # self.lca2 = AT_LCA(dim=self.in_channels[1], num_heads=4)
        # self.lca1 = AT_LCA(dim=self.in_channels[0], num_heads=4)
        ...
        # self.output_layer = nn.Sequential(
        #     BaseConv(3 * self.in_channels[0], self.in_channels[0], 1, 1, activation=nn.ReLU(inplace=True)),
        #     BaseConv(self.in_channels[0], 1, 3, 1),
        # )
    ...
    def forward(self, x):
        ...
        # print('convnext time:', time.time() - start_time)
        # start_time = time.time()

        conv_stem = self.stem_conv(x)
        # print('conv_stem time:', time.time() - start_time)

        # conv_stem = self.stem_conv1(x)
        # conv_stem = self.stem_WTConv(conv_stem) + conv_stem
        # start_time = time.time()
        conv1 = self.stage1(conv_stem)  # C
        # print('conv1 time:', time.time() - start_time)
        # print(f"conv1 shape: {conv1.shape}")
        # start_time = time.time()
        # conv2 = self.stage2(self.down2(conv1))  # 2C
        conv2 = self.down2(conv1)
        # print('down2 time:', time.time() - start_time)
        # start_time = time.time()
        conv2 = self.stage2(conv2)  # 2C
        # print('conv2 time:', time.time() - start_time)
        ...
        # print('down3 time:', time.time() - start_time)
        # start_time = time.time()
        conv3 = self.stage3(conv3)  # 4C
        # print('conv3 time:', time.time() - start_time)
        ...
        # print('down4 time:', time.time() - start_time)
        # start_time = time.time()
        conv4 = self.stage4(conv4)  # 4C
        # print('conv4 time:', time.time() - start_time)
        # print(f"conv4 shape: {conv4.shape}")

        # start_time = time.time()
        mscm4 = self.mscm4(conv4)
        # print('mscm4 time:', time.time() - start_time)
        # mscm4 = self.mscm4(self.catfuse4(conv4))
        # print(f"mscm4 shape: {mscm4.shape}")
        # start_time = time.time()
        mscm4_up = self.up4(mscm4)  # 4C
        # print('mscm4_up time:', time.time() - start_time)
        # print(f"mscm4_up shape: {mscm4_up.shape}")

        # start_time = time.time()
        mscm3 = self.mscm3(conv3)
        # print('mscm3 time:', time.time() - start_time)
        # mscm3 = self.mscm3(self.lca3(conv3, self.fuse_up4(conv4)))
        # mscm3 = self.mscm3(self.catfuse3(conv3))
        # print(f"mscm3 shape: {mscm3.shape}")
        # start_time = time.time()
        de3 = self.de3(mscm3 + mscm4_up)
        # print('de3 time:', time.time() - start_time)
        # print(f"de3 shape: {de3.shape}")
        # start_time = time.time()
        de3_up = self.up3(de3)  # 2C
        # print('de3_up time:', time.time() - start_time)
        # print(f"de3_up shape: {de3_up.shape}")

        # start_time = time.time()
        mscm2 = self.mscm2(conv2)
        # print('mscm2 time:', time.time() - start_time)
        # mscm2 = self.mscm2(self.lca2(conv2, self.fuse_up3(conv3)))
        # mscm2 = self.mscm2(self.catfuse2(conv2))
        # print(f"mscm2 shape: {mscm2.shape}")
        # start_time = time.time()
        de2 = self.de2(mscm2 + de3_up)
        # print('de2 time:', time.time() - start_time)
        # print(f"de2 shape: {de2.shape}")
        # start_time = time.time()
        de2_up = self.up2(de2)  # C
        # print('de2_up time:', time.time() - start_time)
        # print(f"de2_up shape: {de2_up.shape}")

        # start_time = time.time()
        mscm1 = self.mscm1(conv1)
        # print('mscm1 time:', time.time() - start_time)
        # mscm1 = self.mscm1(self.lca1(conv1, self.fuse_up2(conv2)))
        # mscm1 = self.mscm1(self.catfuse1(conv1))
        # print(f"mscm1 shape: {mscm1.shape}")
        # start_time = time.time()
        de1 = self.de1(mscm1 + de2_up)
        # print('de1 time:', time.time() - start_time)
        # print(f"de1 shape: {de1.shape}")

        # ssa4 = self.ssa4(self.ssa_down4_1(self.ssa_down4_2(conv4)))
        # ssa3 = self.ssa3(self.ssa_down3_1(self.ssa_down3_2(conv3)) + self.ssa_up4(ssa4))
        # ssa2 = self.ssa2(self.ssa_down2_1(self.ssa_down2_2(conv2)) + self.ssa_up3(ssa3))
        # ssa1 = self.ssa1(self.ssa_down1_1(self.ssa_down1_2(conv1)) + self.ssa_up2(ssa2))
        # ssa1 = self.ssa_final_up_1(self.ssa_final_up_2(ssa1))

        # ssa = self.ssa_up2(self.ssa_up1(self.ssa(self.ssa_down2(self.ssa_down1(x)))))
        # ssa = self.ssa_up3(self.ssa_up2(self.ssa_up1(self.ssa(self.ssa_down3(self.ssa_down2(self.ssa_down1(x)))))))



        # start_time = time.time()
        output = self.output_layer(torch.cat([de1, convnext], dim=1))
        # print('output time:', time.time() - start_time)
        # output = self.output_layer(torch.cat([de1, convnext, ssa1], dim=1))
        # output = self.output_layer(torch.cat([de1, convnext, ssa], dim=1))
