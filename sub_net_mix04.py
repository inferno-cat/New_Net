...
class UHNet(nn.Module):
    def __init__(self, M=32):
        super(UHNet, self).__init__()
        self.M = M
        self.conv1x1 = nn.Conv2d(3, M, 1, 1, 0)
        self.PDDP1 = nn.Sequential(
            PDDPBlock(M, M // 2, M),
            PDDPBlock(M, M // 2, M),
            PDDPBlock(M, M // 2, M),
            PDDPBlock(M, M // 2, M),
        )
        self.pool1 = PoolBlock(M, M * 2)
        self.PDDP2 = nn.Sequential(
            PDDPBlock(M * 2, M, M * 2),
            PDDPBlock(M * 2, M, M * 2),
            PDDPBlock(M * 2, M, M * 2),
            PDDPBlock(M * 2, M, M * 2),
        )
        self.pool2 = PoolBlock(M * 2, M * 4)
        self.PDDP3 = nn.Sequential(
            PDDPBlock(M * 4, M * 2, M * 4),
            PDDPBlock(M * 4, M * 2, M * 4),
            PDDPBlock(M * 4, M * 2, M * 4),
            PDDPBlock(M * 4, M * 2, M * 4),
        )
        self.fblock3 = nn.Sequential(
            # Upsampling(M * 4, M * 2),
            DySampleFusion_WithOut(M * 4, M * 2),
            FBlock(M * 2, M, M * 2),
        )
        self.fblock2 = nn.Sequential(
            # Upsampling(M * 2, M),
            DySampleFusion_WithOut(M * 2, M),
            FBlock(M , M // 2, M),
        )
        self.fblock1 = nn.Sequential(
            FBlock(M , M // 2 , 1),
        )
    def forward(self, x):
        x = self.conv1x1(x)
        ...
        pdd1 = self.PDDP1(x)
        pdd2 = self.PDDP2(self.pool1(pdd1))
        pdd3 = self.PDDP3(self.pool2(pdd2))
        f3 = self.fblock3(pdd3)
        f2 = self.fblock2(f3+pdd2)
        f1 = self.fblock1(f2+pdd1)

        return torch.sigmoid(f1)
