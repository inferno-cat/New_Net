import os
import torch
from sub_utils import AverageMeter
from tqdm import tqdm
import torchvision
# from sub_dataset import crop_bsds, pad_nyud
...
def train_bsds(train_loader, model, opt, lr_schd, print_freq, max_epoch, epoch, save_dir, logger, device, loss, AT_gradient_accumulation_steps=-1):
    # 创建目录
    # import time
    # t1 = time.time()
    if not os.path.isdir(save_dir):
    ...
    for batch_index, data in enumerate(progress_bar):
        ...
        # 根据AT_gradient_accumulation_steps决定是否累积梯度
        if AT_gradient_accumulation_steps > 1:
            # 累积损失（归一化）
            accumulated_loss += batch_loss / AT_gradient_accumulation_steps
            accumulation_counter += 1

            # 如果达到累积步数或最后一个batch，进行优化
            if accumulation_counter == AT_gradient_accumulation_steps or batch_index == len(train_loader) - 1:
                # 清零梯度
                opt.zero_grad()
                # 反向传播
